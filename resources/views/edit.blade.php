...
@section('content')
    <div class="container mt-5">
        <form class="row g-3 d-flex justify-content-center" action="/listing/{{ $listing->id }}" method="POST"
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="title" class="form-control" id="inputPassword4"
                    placeholder="Software engineer" value="{{ $listing->title }}">
                @error('title')
                    <span style="color: rebeccapurple">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="col-12">
                ...
                <input type="url" name="website" class="form-control" id="inputAddress2"
                    placeholder="https://getbootstrap.com/" value="{{ $listing->website }}">
                @error('url')
                    <span style="color: rebeccapurple">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="col-md-12">
                ...
                <textarea name="description" class="form-control" id="inputZip" cols="30" rows="10"
                    placeholder="Lorem Ipsum">{{ $listing->description }}</textarea>
                @error('description')
                    <span style="color: rebeccapurple">{{ $message }}</span>
                @enderror
            </div>
